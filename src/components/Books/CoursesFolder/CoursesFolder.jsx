...
console.log(firstSemesterCourses);
    ...
    return(
       ...
       <div className="text-center my-[20px] ">
        <h1 className="font-bold uppercase text-slate-900 text-[25px] md:text-[30px] ">{clickedLevel}level e-book Download</h1>
        <p className="text-slate-700 capitalize text-[20px]  ">Here you find/download textbook, handouts and past exam questions for level Courses</p>
       </div>
       ...
       <div className="grid grid-cols-2  gap-2 md:grid-cols-3 my-[10px] ">
        {firstSemesterCourses.map(courses => {
            return <Link onClick={() => getPdf(courses.Course)} to='/download pdf' className="flex flex-col  items-center shadow-2xl hover:bg-slate-900 min-h-[100px] justify-between bg-slate-700 rounded-[2px] px-[10px] py-[20px] ">
                  <FcOpenedFolder className="text-[50px]  font-semibold "/>
              <h1 className="text-[18px] font-semibold text-slate-100 ">{courses.Course}</h1>
        </Link >
        })}
       </div>
</div>
...
<div className="w-full ">
       ...
       {secondSemesterCourses.map(courses => {
            return <Link onClick={() => getPdf(courses.Course)} to='/download pdf' className="flex flex-col  items-center shadow-2xl hover:bg-slate-900 min-h-[100px] justify-between bg-slate-700 rounded-[2px] px-[10px] py-[20px] ">
            <FcOpenedFolder className="text-[50px] text-yellow-500 font-semibold "/>
      <h1 className="text-[18px] font-semibold text-slate-100 ">{courses.Course}</h1>
  </Link >
        })}
